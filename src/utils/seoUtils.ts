...
export function normalizeImageUrl(
  imageUrl: string,
  baseUrl = "https://www.silsilaeiftekhari.in"
): string {
...
export function optimizeImageForPlatform(
  imageUrl: string,
  platform: "whatsapp" | "facebook" | "twitter" = "whatsapp"
): string {
  const normalized = normalizeImageUrl(imageUrl);

  // 🔥 Proxy through your domain for social platforms
  const encoded = encodeURIComponent(normalized);
  return `https://www.silsilaeiftekhari.in/api/og-image?url=${encoded}`;
}
