...
//import { useBookBySearchQuery } from "@framework/product/get-all-books-by-search";
...
export default function SearchTopBar() {
  ...
  //const category = typeof query.categories === "string" ? query.categories : "";

  // 📦 Get books by category
  // const { data } = useBookBySearchQuery({ category });

  // const totalBooks = data?.pages?.[0]?.paginatorInfo?.totalBooks ?? 0;
    ...
    <div className="flex justify-between items-center mb-7">
      ...
      <div className="flex items-center justify-end">
        <div className="flex-shrink-0 text-body text-xs md:text-sm leading-4 ltr:pr-4 rtl:pl-4 ltr:md:mr-6 rtl:md:ml-6 ltr:pl-2 rtl:pr-2 hidden lg:block">
          {count.toLocaleString()} {t("text-items")}
        </div>
        <ListBox
          options={[
